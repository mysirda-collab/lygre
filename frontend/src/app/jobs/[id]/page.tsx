...
export default function JobDetailPage() {
  ...
  return <div className="space-y-5 p-4">
    ...
    <section className="rounded-xl border bg-white p-4"><h2 className="mb-2 font-semibold">Poznámky</h2><div className="flex gap-2"><textarea className="min-h-20 flex-1 border p-2" value={note} onChange={e=>setNote(e.target.value)} placeholder="Přidat poznámku"/><button className="border px-3" onClick={addNote}>Přidat</button></div><ul className="mt-3 space-y-2">{detail.notes.map(item=><li key={item.id} className="border-t pt-2"><time className="text-xs text-slate-500">{new Date(item.created_at).toLocaleString()}</time><p>{item.text}</p></li>)}</ul></section>
